...
// in simulator we redefine to format for sqlrestdemo Developer console
...
{\
	NSString * exceptionName = [@"org.sqlrestdemo." stringByAppendingString:NSStringFromClass([self class])];\
	TiExceptionThrowWithNameAndReason(exceptionName,reason,subreason,location);\
}\
\
+ (void) throwException:(NSString *) reason subreason:(NSString*)subreason location:(NSString *)location\
{\
	NSString * exceptionName = @"org.sqlrestdemo";\
	TiExceptionThrowWithNameAndReason(exceptionName,reason,subreason,location);\
}\
...
enum {
	UIsqlrestdemoNativeItemNone = -1, 
	UIsqlrestdemoNativeItemSpinner = -2,
	UIsqlrestdemoNativeItemProgressBar = -3,
	
	UIsqlrestdemoNativeItemSlider = -4,
	UIsqlrestdemoNativeItemSwitch = -5,
	UIsqlrestdemoNativeItemMultiButton = -6,
	UIsqlrestdemoNativeItemSegmented = -7,
	
	UIsqlrestdemoNativeItemTextView = -8,
	UIsqlrestdemoNativeItemTextField = -9,
	UIsqlrestdemoNativeItemSearchBar = -10,
	
	UIsqlrestdemoNativeItemPicker = -11,
	UIsqlrestdemoNativeItemDatePicker = -12,
	
	UIsqlrestdemoNativeItemInfoLight = -13,
	UIsqlrestdemoNativeItemInfoDark = -14,
	
	UIsqlrestdemoNativeItemDisclosure = -15,
	
	UIsqlrestdemoNativeItemContactAdd = -16
};
...
/**
 *	TiThreadPerformOnMainThread should replace all sqlrestdemo instances of
